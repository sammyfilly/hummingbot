...
def fee_overrides_dict() -> Dict[str, ConfigVar]:
    ...
    for name in AllConnectorSettings.get_connector_settings().keys():
        all_configs |= {
            f"{name}_percent_fee_token": new_fee_config_var(
                f"{name}_percent_fee_token", type_str="str"
            ),
            f"{name}_maker_percent_fee": new_fee_config_var(
                f"{name}_maker_percent_fee", type_str="decimal"
            ),
            f"{name}_taker_percent_fee": new_fee_config_var(
            ...
            f"{name}_maker_fixed_fees": new_fee_config_var(
                f"{name}_maker_fixed_fees", type_str="list"
            ),
            f"{name}_taker_fixed_fees": new_fee_config_var(
